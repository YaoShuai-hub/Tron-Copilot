...
如果不知道答案，直接说不知道。"""
        ...
        try:
            ...
            def _extract_json_blocks(text: str) -> tuple[str, list[str]]:
                if not text:
                    return text, []
                matches = json_pattern.findall(text)
                if not matches:
                    return text, []
                cleaned = json_pattern.sub("", text).strip()
                blocks = [f"<<<JSON\n{json_content}\nJSON>>>" for json_content in matches]
            ...
            if tool_calls:
                ...
                for tc in tool_calls:
                    ...
                    if fn_name == "transfer_tokens":
                        ...
                        step_result, blocks = _extract_json_blocks(step_result or "")
                        if blocks:
                            tool_json_blocks.extend(blocks)
                        if step_result:
                        ...
                        result_str, blocks = _extract_json_blocks(result_str or "")
                        if blocks:
                            tool_json_blocks.extend(blocks)
